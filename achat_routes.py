...
@login_required
def list_achats():
    """Liste tous les achats avec filtre par période et fournisseur"""
    try:
        ...
        achats = query.order_by(desc(Achat.date_achat)).paginate(page=page, per_page=20, error_out=False)
        
        produits = Produit.query.filter_by(actif=True).order_by(Produit.nom).all()
        
        fournisseurs = [
            f[0] for f in db.session.query(Achat.fournisseur)
                                    .distinct()
                                    .filter(Achat.fournisseur.isnot(None), Achat.fournisseur != '')
                                    .all()
        ]
...
def nouvel_achat():
    ...
    try:
        ...
        flash(message, "success" if achat else "error")
...
def api_daily_purchases():
    """API pour les achats quotidiens"""
    try:
...
def list_fournisseurs():
    ...
    try:
        ...
        return render_template('achats.html', show_fournisseurs=True, fournisseurs_stats=top_fournisseurs)
    except Exception as e:
